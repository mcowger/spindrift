...
class CNC:
    ...
    def _parse_status_field(self, key: str, value: str):
        ...
        try:
            if key == 'MPos':
                ...
                self.feed_info.current = float(feed_parts[0]) if len(feed_parts) > 0 else 0.0
                self.feed_info.target = float(feed_parts[1]) if len(feed_parts) > 1 else 0.0
                self.feed_info.override = int(float(feed_parts[2])) if len(feed_parts) > 2 else 100
                
            elif key == 'S':
                # Spindle information
                spindle_parts = value.split(',')
                self.spindle_info.current_rpm = float(spindle_parts[0]) if len(spindle_parts) > 0 else 0.0
                self.spindle_info.target_rpm = float(spindle_parts[1]) if len(spindle_parts) > 1 else 0.0
                self.spindle_info.override = int(float(spindle_parts[2])) if len(spindle_parts) > 2 else 100
                self.spindle_info.vacuum_mode = int(spindle_parts[3]) if len(spindle_parts) > 3 else 0
                self.spindle_info.temperature = float(spindle_parts[4]) if len(spindle_parts) > 4 else 0.0
                self.spindle_info.bed_temperature = float(spindle_parts[5]) if len(spindle_parts) > 5 else 0.0
                
            elif key == 'T':
                # Tool information
                tool_parts = value.split(',')
                self.tool_info.current_tool = int(tool_parts[0]) if len(tool_parts) > 0 else -1
                self.tool_info.tool_length_offset = float(tool_parts[1]) if len(tool_parts) > 1 else 0.0
                self.tool_info.target_tool = int(tool_parts[2]) if len(tool_parts) > 2 else -1
                ...
                self.laser_info.mode = int(laser_parts[0]) if len(laser_parts) > 0 else 0
                self.laser_info.state = int(laser_parts[1]) if len(laser_parts) > 1 else 0
                self.laser_info.testing = int(laser_parts[2]) if len(laser_parts) > 2 else 0
                self.laser_info.power = float(laser_parts[3]) if len(laser_parts) > 3 else 0.0
                self.laser_info.scale = float(laser_parts[4]) if len(laser_parts) > 4 else 100.0
    ...
    def _calculate_wco(self):
        ...
        self.work_coordinate_offset.x = round(self.machine_position.x - 
                                            (math.cos(math.radians(self.rotation_angle)) * self.work_position.x - 
                                             math.sin(math.radians(self.rotation_angle)) * self.work_position.y), 3)
        self.work_coordinate_offset.y = round(self.machine_position.y - 
                                            (math.sin(math.radians(self.rotation_angle)) * self.work_position.x + 
                                             math.cos(math.radians(self.rotation_angle)) * self.work_position.y), 3)
        self.work_coordinate_offset.z = round(self.machine_position.z - self.work_position.z, 3)
        self.work_coordinate_offset.a = round(self.machine_position.a - self.work_position.a, 3)
    ...
    def set_work_coordinate(self, x: Optional[float] = None, y: Optional[float] = None,
                           z: Optional[float] = None, a: Optional[float] = None) -> bool:
        """
    ...
    def __str__(self) -> str:
        ...
        return (f"CNC State: {self.state.value}\n"
